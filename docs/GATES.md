...
| System order (declaration)    | `simulation.ts` `SYSTEM_ORDER`      | An edit to the declared sequence                        | No. The order is data and a test asserts the list                                                                                         | Reordering the literal fails. **Only that** — see the two rows below (`93cbfe4`)                                                            |
...
| System order (execution)      | `determinism.test.ts`               | A silent reordering inside `stepWorld`                  | **No, now.** Was yes: asserting the declaration is not asserting the behaviour, and reversing the loop passed 732 tests                   | Reversed iteration — fails. The observer reports the loop **variable**, so a rewiring of the table does not reach it                        |
| System dispatch               | `runtime/test/systems.test.ts`      | A name wired to a different function                    | No. Each system is reached through `SYSTEMS` by the name the loop uses, and asserted on its effect                                        | `drag: integrate, integrate: applyDrag` — identical observed sequence, hash `89f827c3` → `405759c4`, 911 tests green; now fails             |
| Simulation physics            | `runtime/test/systems.test.ts`      | A neutered constant or a dropped guard clause           | No. Expectations are literals worked out by hand, not recomputed from the constant, so a mutated constant cannot move both sides          | Drag → 1, restitution → 0, bounds → ±1e9, the `controlled` guard, a doubled step, a non-neutral input frame — six survivors, now all killed |
...
| Role          | Verdict      | Artifact                                              | Notes                                                                    |
| ------------- | ------------ | ----------------------------------------------------- | ------------------------------------------------------------------------ |
| QA Automation | **unsigned** | pass 2 report at `8798779`: FAIL, 6 blocking, 8 major | pass 1 at `24ea825`: FAIL, 9 blocking. All addressed; pass 3 not yet run |
| Visual QA     | **unsigned** | pass 2 report at `8798779`: FAIL, 6 blocking, 3 major | pass 1 at `24ea825`: FAIL, 8 blocking. All addressed; pass 3 not yet run |
| Performance   | **unsigned** | pass 2 report at `8798779`: FAIL, 5 blocking, 5 major | pass 1 at `24ea825`: FAIL, 5 blocking. All addressed; pass 3 not yet run |
...
| 8   | Visual QA     | The comparator has a measured colour blind spot on flat-shaded content — 14.5% channel drift passes all three gates                       | **Corrected at pass 2.** The recorded numbers were measured on the pre-aspect-fix build and never re-measured: 11.9% was really 7.43/7.93/4.85%, and the table had no profile column, which hid that the blind spot is absent on the phone. Re-measured per profile in GAP-002; `render.spec.ts` now judges the modal sprite colour at ±8 |
...
| 12  | QA Automation | The guard-audit row "any reorder fails the assertion" was false — reversing `stepWorld`'s iteration passed 732 tests                      | **Incomplete at pass 2.** An observer row was appended and the false row was left standing, so the table still said "any reorder fails the assertion" while a dispatch rewiring passed 911 tests. Row 96 now says what asserting the list catches and what it does not; two rows below it cover execution and dispatch                    |
...
### Blocking findings from pass 2, and their resolutions

Pass 2 reviewed the fixes for pass 1 and **all three roles failed again**. The
findings are not a second helping of the first set: they are almost entirely
about _the guards written to close pass 1_, and the pattern is worth stating
before the table, because it is the finding.

**A guard written in the same pass as the thing it guards shares its blind
spot.** Every mechanism added after pass 1 was defective in the direction that
made it look correct — the mutation sweep could not report a survivor, the
assertion checker accepted a one-letter reference, the tree check honoured
`.gitignore`, the findings filter ate two thirds of real findings, and the
claims ledger read one citation in thirty. Each passed its own tests. None of
them was found by anything in this repository; all were found by a reader
attacking them from outside with a machine.

| #   | Role          | Finding                                                                                                   | Resolution                                                                                                                                 |
| --- | ------------- | --------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | QA Automation | The mutation sweep's kill signal was its own anchor test, so every unit mutation read `killed` regardless | Anchors read from `HEAD`, which a sweep never writes; the control runs unconditionally; the sweep refuses a red baseline (`readCommitted`) |
| 2   | QA Automation | The step observer reports the loop variable, so rewiring `SYSTEMS` changed the world with 911 tests green | `SYSTEMS` exported and each system asserted on its effect through it (`systems.test.ts`)                                                   |
| 3   | QA Automation | The guard-audit row "any reorder fails the assertion" was still present and still false                   | Row 96 restated as declaration-only; rows added for execution and dispatch                                                                 |
| 4   | Both          | Dropping `presentMs` under-reported the gated frame cost by 90% with the whole suite green                | Composition declared as `CPU_FRAME_TERMS`/`CPU_FRAME_EXCLUDED` and audited field by field; every fixture had zeroed the term               |
| 5   | QA Automation | Reverting the bundle split, or zeroing its share floor, left everything green                             | Both moved into `splitBundle` in the library, with tests; `packages/core` was also missing from the chunk predicate                        |
| 6   | QA Automation | `review-findings.ts` rejected six of nine legitimate findings                                             | Filter made grammatical: reader-directed and injection patterns anywhere, tooling imperatives sentence-initial only                        |
| 7   | Visual QA     | `resize` neuterable — the old assertion was satisfied _harder_ by the defect (0.959 vs 0.787)             | Canvas geometry asserted against the viewport; scene centring and coverage after rotation                                                  |
| 8   | Visual QA     | The rotation test read the canvas size and asserted nothing; RC-0013 caught only by a conjunction         | Both defences now individually load-bearing                                                                                                |
| 9   | Visual QA     | Collapsing every sprite to y=0 passed; 50 of 400 sprites passed                                           | Coverage derived from the scene (formula matches the reviewer's measurement to three digits) and a block-span assertion                    |
| 10  | Visual QA     | The parity spec compared a frame with itself, so `compareImages → ok: true` survived                      | A negative control: the same comparator against a damaged copy of the same real capture                                                    |
| 11  | Visual QA     | GAP-002's numbers were measured on the pre-aspect-fix build and had no profile column                     | Re-measured per profile; 11.9% was really 7.43/7.93/4.85%, and the blind spot is absent on the phone                                       |
| 12  | Performance   | Six E2E tests failed under the load this repo's own review procedure creates                              | Contended control legs discarded; five pairs; contention diagnosed separately from missing throttling                                      |
| 13  | Performance   | The DPR inverse control had no power: one-sided assertion, 42% headroom                                   | A floor on the budget so it can fail downward; residual sensitivity recorded; the decisive control named as the unit one                   |
| 14  | Performance   | Sensitivity wrong by an order of magnitude, in four documents that disagreed                              | One table: 6% / 6% / 88%, 4.66ms against 8ms. A 12x simulation regression passes                                                           |
| 15  | Performance   | `DROPPED_FRAME_MS` hardcoded to 60Hz inverts the check at 120Hz                                           | Threshold inferred from the run's own fastest frame, capped so it is never more lenient                                                    |
| 16  | Performance   | The floor rule keyed on unit, so `soak.heapGrowth.ratio` had no floor                                     | Generalised by shape; `min: 0` allowed only where the rule says why zero is real                                                           |
| 17  | QA Automation | The self-test's completeness check compared two hardcoded lists in one file                               | Registry in `detectors.ts`, checked against the CLI directory; three scenarios added for `checkProfileOrdering`                            |

Majors resolved alongside: the assertion checker's one-letter references,
whole-line quote skipping and filename-shaped exemption; the tree check's
`.gitignore` blindness; the shell-edit ban's opt-out marker and three unmodelled
write forms; the claims ledger's coverage; the mesh-count cross-check comparing
two literals on one profile; measurements recorded before their honesty
assertions; a third copy of the upper-middle median; the empty page described as
a positive control it cannot be; `pnpm sweep` never running the mutation sweep;
and the cold-load spec running on a 30-second default.
